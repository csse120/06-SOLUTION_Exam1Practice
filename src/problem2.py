...
#         upper_left.move_by(-delta, -delta)
#         lower_right.move_by(delta, delta)
    ...
    # At one point in history, we disabled the Point's move_by and
    # move_to for the sake of this problem, where misuse of them
    # can cause a subtle error.
    #         upper_left.move_by(-delta, -delta)
    #         lower_right.move_by(delta, delta)
    # where each Rectangle is constructed from the SAME point objects.
    # We no longer need to do so, since now Rectangle (and all other
    # similar objects) CLONE the points from which they are constructed.
