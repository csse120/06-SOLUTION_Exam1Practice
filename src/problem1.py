...
Authors: David Mutchler, Vibha Alangar, Valerie Galluzzi, Mark Hays,
         Amanda Stouder, their colleagues and SOLUTION by David Mutchler..
...
def problem1a(m, n):
    ...
    for k in range((n * n) - (m * m) + 1):
        total = total + math.sin(k + (m * m))  # -> m**2, m**2 + 1, ...
...
def run_test_problem1b():
    ...
    # Test 4:
    expected = 3
    answer = problem1b(4, 3)
    print()
    print('Test 4 expected:', expected)
    print('       actual:  ', answer)

    # NOTE: Students were required to do at least  4  tests,
    # so the following one is unnecessary.
    # In fact, it would be WRONG  if included in a student answer,
    # since there is (presumably) no way to compute the answer other
    # than seeing what the code produces.

    # Test 5 (takes about 10 seconds to run on my laptop):
    expected = 5038
    answer = problem1b(500, 100)
    print()
...
def problem1b(m, f):
    ...
    for k in range((f * m) - m + 1):
        if is_prime(k + m):  # -> m, m+1, m+2 ...
            count = count + 1

    return count

    # Students might prefer  is_prime(k + m) == True   in the above.
